...
run;
%MEND;
%MACRO AtRisk (in_data=, out_data=, outcome=, exposure_var=, fu_stt_var=, fu_end_dt=, note=);
data temp_0;
set &in_data.;
time = min(&outcome._dt, &fu_end_dt.) - &fu_stt_var.;
if &outcome._dt^="." and &outcome._dt= min(&outcome._dt, &fu_end_dt.)
	then event=1; else event=0;
rename &exposure_var.=exposure;
run;
proc sort data=temp_0 out=temp_1; by exposure time; run;

data temp_2;
set temp_1;
by exposure time;
retain intrtDurationOrd;
if first.exposure then intrtDurationOrd=1;
else intrtDurationOrd=intrtDurationOrd+1;
if last.exposure then call
symput('trttot'||cat(exposure),intrtDurationOrd);
run;

proc sort data=temp_2; by exposure time; run;
data temp_3;
set temp_2;
by exposure time;
if exposure=1 then atrisk=&trttot1. - intrtDurationOrd +1;
if exposure=0 then atrisk=&trttot0. - intrtDurationOrd +1;
if first.time=1 then output;
run;

data temp_4(rename=(atrisk=atrisk_int)) temp_5(rename=(atrisk=atrisk_comp));
set temp_3(keep=exposure time atrisk);
note="&note.";
if exposure=1 then output temp_4;
else output temp_5;
drop exposure;
run;
...
select time, note
, (case when time=1 then max(atrisk_int) else atrisk_int end) as atrisk_int
, (case when time=1 then max(atrisk_comp) else atrisk_comp end) as atrisk_comp
from temp_6;
quit;

proc stdize data=temp_7 out=temp_8 reponly missing=0; run;

data temp_9;
set temp_8;
if first.time then do; flag_1st_int=1; flag_1st_comp=1; end;
if atrisk_int=0 then flag_1st_int=0; else flag_1st_int=1;
if atrisk_comp=0 then flag_1st_comp=0; else flag_1st_comp=1;
retain change_int 0 change_comp 0;
change_int= change_int+flag_1st_int;
change_comp= change_comp+flag_1st_comp;
run;

proc sql;
create table temp_10 as
select distinct *, max(atrisk_int) as atrisk_int1
from temp_9
group by change_int;

create table temp_11 as
select distinct *, max(atrisk_comp) as atrisk_comp1
from temp_10
group by change_comp;
quit;

data temp_12;
set temp_11;
keep time atrisk_int1 atrisk_comp1 note;
rename atrisk_int1=atrisk_int atrisk_comp1=atrisk_comp;
run;

data &out_data.; set temp_12; run;
proc delete data=temp_0 - temp_12; run;
%MEND;
